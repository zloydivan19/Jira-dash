...
**ОС:** любая (Linux/Windows/macOS) — чистый Node.js/Express, нативных
бинарных зависимостей нет.

**Софт на сервере:**
- Node.js 18+ (проверено на 22.x), npm — для сборки и запуска
- systemd или pm2 (или аналог) — чтобы процесс переживал перезапуск/падения
- Реверс-прокси (nginx или аналог) — для TLS, см. раздел ниже

**Сеть:**
- Исходящий доступ с сервера до `https://<домен>.atlassian.net` (443/tcp) — сервер ходит в Jira REST API от имени пользователя
- Входящий доступ от пользователей до реверс-прокси (443/tcp снаружи → порт приложения локально, см. «Конфигурация»)
- Внешний интернет самому Node-процессу не нужен, кроме доступа к Jira

**Ресурсы:** приложение лёгкое — Express-прокси без базы данных и без
хранения состояния между запросами (все данные каждый раз запрашиваются из
Jira заново). Ориентировочно достаточно 1 vCPU / 512 МБ RAM. Место на диске:
исходники + `node_modules` ~110 МБ, собранный `dist/` ~1.5 МБ.

**Хранилище/БД:** не требуется — приложение полностью stateless, ничего не
пишет на диск и не хранит в памяти между запросами.

**Клиент (браузер пользователя):** любой современный evergreen-браузер
(Chrome/Edge/Firefox последних версий) — используется React 18 и ES-модули,
поддержка IE не нужна.

...
const STEPS = [
  { tab: 'connection', title: 'Добро пожаловать в PM Radar',
  ...
  { tab: 'eval', target: 'nav-eval', title: 'Контроль оценки',
    text: 'Все CR, которые сейчас на оценке, с отсчётом срока по SLA. Задачи сгруппированы: в процессе, на паузе и уже оценённые.' },
  { tab: 'eval', target: 'eval-sla', title: 'SLA оценки',
    text: 'На всю оценку даётся 10 рабочих дней. Считаются только дни в статусах оценки: модерация, «На оценку», «Уточнение требований», Product Feature. Пауза и «Отложено» останавливают отсчёт. Зелёный: до 5 дней, жёлтый: 6–8, красный: больше 8. На модерацию свой срок: до 2 дней норма, больше 3 нарушение.' },
  { tab: 'eval', target: 'eval-ping', title: 'Пинги',
    text: 'Отметьте задачи галочками в таблице и нажмите эту кнопку. Напишите одно сообщение, и оно уйдёт комментарием во все отмеченные задачи. В каждой задаче будет упомянут её исполнитель, и Jira пришлёт ему уведомление.' },
  { target: 'nav-bugControl', title: 'Контроль ошибок',
    text: 'Ваши ошибки и сдвиги версии исправления: с какого релиза на какой перенесли и сколько раз.' },
  { target: 'nav-ttm', title: 'TTM анализ',
    text: 'Сколько времени CR проходит от создания до релиза, по командам и по фазам.' },
  { target: 'nav-fields', title: 'Поля таблиц',
    text: 'Какие колонки показывать в каждом разделе. Стандартный набор можно вернуть одной кнопкой.' },
  { target: 'help', title: 'Этот тур',

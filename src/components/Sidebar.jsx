...
export default function Sidebar({
  ...
  const tabBtn = (tab) => ({
    ...
    textAlign: 'left',
    background: activeTab === tab ? (theme.id === 'csi' ? '#e8f0f8' : '#1a2e40') : 'transparent',
    color: activeTab === tab ? (theme.id === 'csi' ? theme.accent : '#ffffff') : theme.textSecondary,
    display: 'flex',
    alignItems: 'center',
    gap: '10px',
  });
          ...
          <div style={{ padding: '14px', flex: 1, minHeight: 0, borderBottom: `1px solid ${theme.borderLight}`, overflowY: 'auto' }}>
            ...
            {ttmHasData && (
              <div style={{ marginTop: '10px', padding: '8px 10px', background: theme.bgInput, border: `1px solid ${theme.borderLight}`, borderRadius: '6px', fontSize: '11px', color: theme.textSecondary }}>
                ...
                {' '}медианный TTM: <b style={{ color: theme.textPrimary }}>{fmtDaysPair(ttmSummary?.median?.cal, ttmSummary?.median?.work)}</b>
              </div>
            )}
          </div>
        </div>
